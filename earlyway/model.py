...
class VGGBase(nn.Module):
    def __init__(self):
        ...
        self.pool1 = nn.MaxPool2d(kernel_size=2, stride=2) #300->150

        self.conv2_1 = nn.Conv2d(128, 128, kernel_size=3, padding=1)
        self.conv2_2 = nn.Conv2d(128, 128, kernel_size=3, padding=1)
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2) #150->75

        self.conv3_1 = nn.Conv2d(128, 256, kernel_size=3, padding=1)
        self.conv3_2 = nn.Conv2d(256, 256, kernel_size=3, padding=1)
        self.conv3_3 = nn.Conv2d(256, 256, kernel_size=3, padding=1) #75->38
    ...
    def forward(self, image_rgb,image_thermal):
        ...
        out_rgb = F.relu(self.conv1_1_rgb(image_rgb))  
        out_rgb = F.relu(self.conv1_2_rgb(out_rgb))
        out_rgb = self.pool1(out_rgb)


        out_thermal = F.relu(self.conv1_1_thermal(image_thermal)) 
        out_thermal = F.relu(self.conv1_2_thermal(out_thermal))
        out_thermal=self.pool1(out_thermal)

        out = torch.cat([out_rgb, out_thermal], dim=1)
    ...
    def load_pretrained_layers(self):
        ...
        state_dict = self.state_dict() #훈련된 모델을 저장
        ...
        state_dict['conv1_2_thermal.weight']=state_dict['conv1_2_rgb.weight']
        state_dict['conv1_2_thermal.bias']=state_dict['conv1_2_rgb.bias']

        state_dict['conv2_1.weight']=torch.cat((state_dict['conv2_1.weight'],state_dict['conv2_1.weight']),dim=1)
